...
Next we pivot both datasets to long format, where each observation corresponds to a country, year, and its mortality rate/income. This allows us to obvserve our data by country-year pairs for observations rather than each observation being independent country.
...
We chose to join from child to income which will keep all the countries child has, but drop those two additional countries' data from Liechtenstein and Holy See.
...
Above, we clearly plotted a better fitted line, suggesting that a log-log model is much more appropriate for our data. Here are the calculated coefficients for the log-log model:
...
We can see that the total variance in log(mortality) is 0.134, where 0.064 is explained by the log(income) and the other 0.07 is unexplained. Indicating that approximately 47.8% of the variation in child mortality rates across countires can be accounted for by their difference in income.
...
Overall, the model explains 47.6% of the variance in child mortality rates across countries (R² = 0.476), indicating a strong relationship between income and child mortality. This statistic and extremely low p-values for both coefficients confirm that this relationship is statistically significant.
...
run_cv <- function(k, data) {
  ...
  cv_fold <- function(fold_index, data) {
    train_data <- data |> filter(fold != fold_index)
    test_data <- data |> filter(fold == fold_index)
    
    model <- lm(log_mortality ~ log_income, data = train_data)
    predictions <- predict(model, newdata = test_data)
    
    r_squared <- cor(test_data$log_mortality, predictions, use = "complete.obs")^2
