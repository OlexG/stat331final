...
Next we pivot the data to long format, allowing us to join and group by country-year pairs for observations. We chose a left join from child to income with will keep all the countries child has, but drop those two additional countries' data from Liechtenstein and Holy See.
