...
```
Seems like 5% of countries have missing income values
up until 1950. These are probably the same countries
as the rate is constant at 5%. Let's figure out which 
countries these are. 
...
## Linear Regression
We will further analyze the relationship between income and child mortality through fitting a linear regression model.
We chose to use median values across all years rather than means because the median is more robust to outliers and extreme values, which one might expect for some countries due to external events (wars increasing child mortality, etc...).
Let's begin by plotting the scatterplot of this data,
where x is our median daily income value, and y is 
the median child mortality value.
...
Our intercept is 2.2384 and our slope is -0.7381. 
For each unit increase in log₁₀(Income), log₁₀(Child Mortality) decreases by 0.7381 units. 
